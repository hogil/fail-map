...
2. Bucket B (Sub): 
   ...
   - Run parallel `list_objects_v2` for each Token prefix (e.g., "YYYYMMDD/TOKEN").
   - Filter files in memory (Time difference 0~10s).
   - Download matched files in parallel.
...
def get_cython_convert_hex():
    ...
    try:
        ...
        # Fallback if cython module is missing (for local testing without build)
        def _fallback(lines, start_idx, xsize, ysize):
            return ""
        return _fallback
...
def create_sample_image_func(args):
    ...
    # [NEW] Extract secondary info if available
    ...
    json_obj = {
        ...
        "stime": stime,
        "day": day,
        "extra_info": extra_info,  # [NEW] Added
        "coord": {
...
class S3Manager:
    ...
    def download_and_decompress_parallel(self, keys):
        # ... (Same as original) ...
        # Simplified for brevity, assuming original logic is preserved
        def _one(key):
            try:
                body = self.client.get_object(Bucket=self.cfg.bucket_name, Key=key)['Body'].read()
                return [(key, body.decode('utf-8', errors='ignore'))] # Simplified decompress
            except: return []
        allc = []
        with ThreadPoolExecutor(max_workers=self.cfg.download_threads) as ex:
            for part in ex.map(_one, keys):
                allc.extend(part)
        return allc
    ...
    def fetch_secondary_data_parallel(self, dataset_A):
        """
        1. Identify unique Tokens and Dates from dataset_A.
        2. Parallel ListObjectsV2 on Secondary Bucket using "Date/Token" prefix.
        ...
        # Group by Token+Date to minimize list requests
        # Assumption: Secondary Bucket Structure = "YYYYMMDD/TOKEN_WAFER_STEP_HHMMSS.json"
        groups = {}
        for d in dataset_A:
            tok = d.get('token')
            stime = d.get('stime') # YYYYMMDD_HHMMSS
            if not tok or not stime: continue
            
            day = stime.split('_')[0] # YYYYMMDD
            prefix = f"{day}/{tok}"   # Optimized Prefix
        ...
        def _list_prefix(prefix):
            # List objects under "YYYYMMDD/TOKEN..."
            try:
        ...
        # 2. In-Memory Filter (Join)
        # Map: (Token, Wafer, Step) -> Dataset Item
        # We need to parse filename of found_files to get Token, Wafer, Step, Time
        
        # Helper to parse secondary filename
        # Expecting: "YYYYMMDD/TOKEN_WAFER_STEP_HHMMSS.json"
        def _parse_sec_name(key):
            # Simple heuristic parser - adjust regex based on actual naming
            bn = os.path.basename(key)
            # Remove ext
            ...
            if len(parts) >= 4:
                ...
                if len(ts_str) == 6 and ts_str.isdigit():
                    return parts[:-1], ts_str
            return None, None

        # Build lookup for dataset_A
        # Key: (Token, Wafer, Step) -> { 'stime_dt': datetime, 'item': d }
        lookup = {}
        for d in dataset_A:
            k = (d.get('token'), d.get('wafer'), d.get('step'))
            dt = _parse_stime_dt(d.get('stime'))
            if dt:
                lookup[k] = {'dt': dt, 'item': d}

        keys_to_download = []
        
        for obj in found_files:
            key = obj['Key']
            parts, ts_str = _parse_sec_name(key)
            if not parts: continue
            
            # Reconstruct Token/Wafer/Step from parts (Heuristic)
            # This part depends heavily on naming convention. 
            # Assuming parts contain Token, Wafer, Step in some order or substring.
            # For now, let's try to match against lookup keys.
            
            matched_entry = None
            
            # Try to match parts against known keys
            # This is O(N*M) but N(files per prefix) and M(dataset per prefix) are small
            
            # Optimization: Create a set of "Token_Wafer_Step" strings from lookup
            # But parts might be split differently.
            
            # Let's assume the filename contains the Wafer and Step explicitly
            fname_base = os.path.basename(key)
            
            for (tok, waf, step), val in lookup.items():
                # Check if Wafer and Step are in filename
                if waf in fname_base and step in fname_base:
                    # Check Time
                    # Construct full datetime for secondary file
                    # We know the 'day' from the prefix or parent folder
                    day = key.split('/')[0] # YYYYMMDD
                    try:
                        dt_sec = datetime.strptime(f"{day}_{ts_str}", "%Y%m%d_%H%M%S")
                        dt_prim = val['dt']
                        diff = (dt_sec - dt_prim).total_seconds()
                        if 0 <= diff <= 10:
                            keys_to_download.append((key, (tok, waf, step)))
                            break # Found match
                    except:
                        pass
...
class ImageGenerator:
    ...
    def generate_images_mixed(self, dataset_all, base_root, positions_root):
        ...
        for (tok, p1, p2, root, step, wafer, stime), samples in groups.items():
            ...
            tasks.append((
                samples, out_path, self.cfg.border_thickness, rot, self.cfg.default_tile_size,
                self.cfg.draw_empty_chip_text, self.cfg.empty_chip_text_field,
                self.cfg.defect_border_thickness, positions_root
            ))
