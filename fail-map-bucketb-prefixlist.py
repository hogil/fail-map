...
def generate_bucket_b_prefixes(info: dict, offset_range=(0, 10)):
    """
    list_objects_v2 를 아주 좁은 prefix로 호출하기 위한 prefix 생성.
    - 자정 넘김을 고려해 offset 시작/끝으로 day 2개까지 생성
    """
    dt0 = datetime.strptime(f"{info['date']}_{info['time']}", "%Y%m%d_%H%M%S")
    dt_a = dt0 + timedelta(seconds=int(offset_range[0]))
    dt_b = dt0 + timedelta(seconds=int(offset_range[1]))
    days = {dt_a.strftime("%Y%m%d"), dt_b.strftime("%Y%m%d")}
    for day in days:
        yield f"{day}/{info['lot_id']}_{info['wafer_w']}_{day}_"
