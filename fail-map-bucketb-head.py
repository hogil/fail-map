...
# =================== Bucket B Matching Helpers (HEAD) ===================
...
class S3ManagerB:
    ...
    def exists(self, key: str) -> bool:
        try:
            self.client.head_object(Bucket=self.cfg.bucket_name, Key=key)
            return True
        except:
            return False
...
def build_bucket_b_match_map_head(part_keys_a, s3b: S3ManagerB, cfg_b: BucketBConfig):
    """
    returns: dict[a_key] -> match_meta
    """
    t0 = time.time()

    def _work_one(ka: str):
        ...
        line = s3b.read_gz_first_line(hit_key, max_bytes=cfg_b.firstline_max_bytes)
        return ka, {
            "matched": True,
            "bucket": cfg_b.bucket_name,
            "method": "head",
            ...
            "time_offset_range": list(cfg_b.time_offset_range),
            "first_line": line,
            "first_line_ok": bool(line),
        }

    match_map = {}
    with ThreadPoolExecutor(max_workers=cfg_b.head_max_workers) as ex:
        for ka, meta in ex.map(_work_one, part_keys_a):
            match_map[ka] = meta

    matched = sum(1 for v in match_map.values() if v.get("matched"))
    ok_first = sum(1 for v in match_map.values() if v.get("matched") and v.get("first_line_ok"))
    print(f"[bucketB(head)] matched={matched}/{len(part_keys_a)}  firstline_ok={ok_first}/{matched if matched else 0}  in {time.time()-t0:.2f}s")
    return match_map
...
def run_pipeline_for_dataframe(df: pd.DataFrame):
    ...
    try:
        ...
        for idx, off in enumerate(range(0, len(matched_keys), chunk_size), 1):
            ...
            # Bucket B 매칭(먼저): A key -> bucket_b_match meta (HEAD 방식)
            bucket_b_match_map = {}
            if s3b:
                bucket_b_match_map = build_bucket_b_match_map_head(part_keys, s3b, CFG_B)
