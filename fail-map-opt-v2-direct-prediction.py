...
# -*- coding: utf-8 -*-
"""
fail-map-opt-v2-direct-prediction.py

Optimization Strategy: Direct Prediction & Async Check (No Listing)
1. Bucket A (Main): Download & Parse.
2. Bucket B (Sub):
   - Do NOT use `list_objects`.
   - For each parsed item, predict possible filenames for +0s to +10s.
   - Fire parallel `get_object` requests for all candidates.
   - Use the first successful download.
3. Merge & Generate Image.

Pros: Extremely fast if filename pattern is predictable.
Cons: Generates 11x requests (0~10s) per item.
"""
...
class PipelineConfig:
    ...
    max_pool_connections: int = 512  # Increased for many small requests
    download_threads: int = 256      # Increased
...
class S3Manager:
    ...
    # [NEW] Direct Prediction Download (No List)
    def download_secondary_direct_prediction(self, dataset_A):
        """
        For each item in dataset_A, generate candidate keys for +0s to +10s.
        Try to download them in parallel.
        """
        if not dataset_A: return {}

        # 1. Generate Candidates
        # Map: candidate_key -> (Token, Wafer, Step)
        candidate_map = {}
...
class DataProcessor:
    ...
    def process_files_parallel_tagged(self, tagged_pairs):
        ...
        results = list(tqdm(self.executor.map(process_file_content, file_contents, chunksize=max(1, len(file_contents)//(self.cfg.cpu_processes*4) or 1)), total=len(file_contents), desc="Processing"))
        out = []
        for idx, fr in enumerate(results):
            tok, p1, p2 = tagged_pairs[idx][0], tagged_pairs[idx][1], tagged_pairs[idx][2]
            for r in fr: r["token"] = tok; r["p1"] = p1; r["p2"] = p2; out.append(r)
        return out
...
def run_pipeline_for_dataframe(df: pd.DataFrame):
    ...
    try:
        ...
        for idx, off in enumerate(range(0, len(matched_keys), chunk_size), 1):
            ...
            # [NEW] Direct Prediction Download
            print("  -> Predicting & Downloading secondary data...")
            t_sec = time.time()
            secondary_data_map = s3.download_secondary_direct_prediction(dataset_all)
